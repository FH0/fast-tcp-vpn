...
### 第二阶段：封装服务（预计 2-3 小时）
- [x] 2.1 实现 `Encapsulator` 封装逻辑
- [x] 2.2 实现 `Encapsulator` 解封装逻辑
- [x] 2.3 单元测试：封装/解封装往返

### 第三阶段：Server 重构（预计 3-4 小时）
- [x] 3.1 重构 `tun_to_socket_worker`
- [x] 3.2 重构 `socket_to_tun_worker`
- [x] 3.3 更新会话查找逻辑
- [x] 3.4 集成测试

### 第四阶段：Client 重构（预计 2-3 小时）
- [x] 4.1 重构 `tun_to_socket_worker`
- [x] 4.2 重构 `socket_to_tun_worker`
- [x] 4.3 集成测试

### 第五阶段：端到端测试（预计 2 小时）
- [x] 5.1 本地回环测试（TCP 流量）
- [x] 5.2 本地回环测试（UDP 流量）
- [x] 5.3 本地回环测试（ICMP ping）
- [x] 5.4 混合流量测试
